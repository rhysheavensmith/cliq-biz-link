...
const LinkGen = () => {
  ...
  const fetchSuggestions = useCallback(async (input) => {
    if (!input || input.trim().length < 3) {
      // Only search if input is >= 3 chars
      setSuggestions([]);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen">
      <div className="max-w-3xl md:w-full py-10 px-24 max-sm:px-10 bg-[#121212] border border-[#ff8d02] rounded-lg">
        {/* Logo */}
        <Image
          src={logo}
          alt="Google"
          className="w-auto h-16 mb-6 max-sm:h-12"
        />

        {/* Autocomplete Input */}
        <div className="mb-4 relative" ref={autocompleteContainerRef}>
          <div className="flex flex-row justify-between gap-2 border-[#ff8d02] border-2 pl-5">
            <input
              type="text"
              value={inputValue}
              onChange={handleInputChange}
              placeholder="Enter business name..."
              className="focus:outline-none flex-1 bg-transparent text-sm max-sm:text-xs"
              onFocus={() => setShowSuggestions(suggestions.length > 0)} // Show suggestions on focus if they exist
            />
            <button
              onClick={handleGenerateLink}
              className="py-3 bg-[#ff8d02] text-white font-semibold text-xl max-md:text-xs flex-shrink-0 px-6 cursor-pointer"
          ...
          {showSuggestions && (
            <ul className="absolute z-10 w-full bg-[#1f1f1f] border border-[#ff8d02] rounded-md mt-1 max-h-60 overflow-y-auto shadow-lg">
              {suggestions.map((suggestion) => (
                <li
                  key={suggestion.place_id}
                  onClick={() => handleSuggestionClick(suggestion)}
                  className="px-4 py-2 text-sm cursor-pointer hover:bg-[#2f2f2f]"
                >
        ...
        <div className="mt-4">
          <label
            htmlFor="reviewLink"
            className="block text-gray-200 font-bold text-2xl my-[28px]"
          >
